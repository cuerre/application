...
<div class="shadow-sm rounded bg-white p-3 {{ $class }}">
    ...
    @if( !is_null($snippet) )
        @if( view()->exists('snippets.' . $snippet) )
            <pre class="bg-white"><code class="language-{{$language}}">@include('snippets.' . $snippet)</code></pre>
        @endif
    @endif
    @unless( empty($slot->__toString()) === true )
        <pre class="bg-white"><code class="language-{{$language}}">{{ html_entity_decode($slot) }}</code></pre>
    @endunless
</div>